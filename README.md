...
# Задача 3. Выделение бизнес-логики из инфраструктурного слоя

## 1. Что архитектурно неудачно

В описанной учебной информационной системе бизнес-правила расчёта скидок, проверки права на льготы и обработки заявок реализованы прямо в контроллерах и сервисах, связанных с HTTP-слоем, ORM, SQL-запросами и внешними API.

Архитектурно это неудачно, потому что бизнес-логика оказывается зависимой от инфраструктурных деталей. Контроллеры, база данных, ORM и внешние API относятся к техническим механизмам реализации и не должны определять правила предметной области.

Из-за такой связанности при изменении схемы базы данных или внешнего API приходится менять не только технический код, но и участки бизнес-логики. Это усложняет сопровождение системы, повышает риск ошибок и делает тестирование бизнес-правил более трудным.

## 2. Подход, который уместен

В данной ситуации уместно применить **чистую архитектуру**.

Чистая архитектура предполагает разделение приложения на слои с чёткими зонами ответственности. Главная идея этого подхода заключается в том, что бизнес-логика и сущности должны находиться во внутренних слоях системы и не зависеть от внешних механизмов: фреймворков, баз данных, ORM, HTTP, SQL-запросов и сторонних API.

Зависимости в такой архитектуре должны быть направлены внутрь: от внешних технических слоёв к внутренним слоям бизнес-логики и сущностей. Внутренние слои не должны содержать прямых ссылок на инфраструктуру.
...
## 3. За счёт чего подход решает проблему

Чистая архитектура решает проблему за счёт отделения бизнес-правил от инфраструктурных деталей.

Правила расчёта скидок, проверки льгот и обработки заявок должны быть вынесены из контроллеров и технических сервисов во внутренний слой бизнес-логики. Контроллеры при этом должны только принимать HTTP-запрос, преобразовывать входные данные и вызывать нужный сценарий использования.

Работа с базой данных, ORM, SQL-запросами и внешними API должна быть вынесена во внешний инфраструктурный слой. Связь между инфраструктурой и бизнес-логикой выполняется через адаптеры и абстракции. При необходимости между слоями могут использоваться DTO, чтобы внутренние модели системы не зависели от формата внешних API или структуры таблиц базы данных.

Например, бизнес-логика не должна напрямую выполнять SQL-запрос для проверки льготы. Вместо этого она обращается к абстракции, например к интерфейсу репозитория. Конкретная реализация репозитория уже находится во внешнем слое и может использовать ORM или SQL.

Благодаря этому при изменении базы данных, ORM или внешнего API изменяется только инфраструктурный слой. Бизнес-правила остаются стабильными и не требуют переписывания.

## 4. Вывод

Основная проблема исходной структуры заключается в том, что бизнес-логика смешана с техническими деталями реализации. Это приводит к высокой связанности, усложняет тестирование и делает систему менее устойчивой к изменениям.

Для решения этой проблемы уместно применить чистую архитектуру. Она разделяет систему на слои, направляет зависимости внутрь и изолирует бизнес-логику от фреймворков, баз данных, HTTP, ORM и внешних API.

В результате систему становится проще развивать, тестировать и сопровождать, так как изменения инфраструктуры не требуют переписывания основных бизнес-правил.
